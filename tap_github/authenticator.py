    ...
    def authenticate_request(
        ...
        while attempts < max_attempts:
            if self.active_token:
                ...
                # No token configured: fall back to unauthenticated access
                # (low rate limit) rather than failing the request.
                logger.info(
            ...
            attempts += 1
            logger.warning(
                f"Retrying authentication attempt ({attempts}/{max_attempts})..."
            )

        raise RuntimeError(
            "Failed to attach a valid GitHub auth token after multiple attempts."
        )
