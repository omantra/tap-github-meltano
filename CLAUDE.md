...
uv run tox                           # run tests across py3.9–3.13 (tox config is in pyproject.toml)
```
...
- Target Python 3.9; use `from __future__ import annotations` and PEP 604 (`X | None`, not `Optional[X]`). Type-only imports go under `if TYPE_CHECKING:` — but keep runtime-used imports (e.g. `requests`) at module level.
